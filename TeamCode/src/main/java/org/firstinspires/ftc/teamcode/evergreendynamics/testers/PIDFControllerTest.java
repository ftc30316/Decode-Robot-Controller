...
public class PIDFControllerTest extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        turretMotor = (DcMotorEx)hardwareMap.get(DcMotor.class, "leftFlywheel");
        ...
        while(opModeIsActive()) {
            ...
            telemetry.addData("Current Velocity", turretMotor.getVelocity());

            TelemetryPacket packet = new TelemetryPacket();
            packet.put("Target Velocity", targetVelocity);
            packet.put("Current Velocity", turretMotor.getVelocity());
            FtcDashboard.getInstance().sendTelemetryPacket(packet);

            if (gamepad1.crossWasPressed()) {
                pidfModified = new PIDFCoefficients(NEW_P, NEW_I, NEW_D, NEW_F);
                turretMotor.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER, pidfModified);
            }
